...
/*   Updated: 2021/04/06 12:28:23 by cabouelw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_hist(t_history **hist)
{
	...
	*hist = NULL;
	fd = open("/Users/cabouelw/.minishell_history", O_RDWR | O_CREAT, 0666);
...
void	history(t_read *s_read, t_history **hist)
{
	...
	if (!s_read->len)
		return ;
	fd = open("/Users/cabouelw/.minishell_history", O_RDWR | O_TRUNC);
	close(fd);
	fd = open("/Users/cabouelw/.minishell_history", O_RDWR | O_APPEND);
	create_node(&node);
